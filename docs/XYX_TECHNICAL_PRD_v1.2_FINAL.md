...
**Primary implementation goal:** Deliver one complete, live, evidence-backed autonomous-agent job lifecycle from human intent to final settlement, including Protected Job COMPLETE and REJECT paths.
...
Protected Job product-level verdicts:
...
Graph/Risk fail-closed applies ONLY during stages that require Graph before an economic action or settlement decision:
- Provider selection: Graph unavailable, stale, or indexing-invalid → FAIL_CLOSED;
- Open Purchase post-payment receipt/evidence indexing failure → EVIDENCE_PIPELINE_INCOMPLETE; reconciliation required.
...
# 26. OUTCOME / COMPLETE / REJECT / FAIL-CLOSED
...
# 32. IPFS VALIDATION CHECKLIST

Section 24 is the single normative source for IPFS behavior and failure semantics. This section defines the validation gate used by implementation and E2E tests.

## 32.1 Required Validation

For every evidence artifact that must be persisted:
...
## 32.2 Mode-Specific Gate

- Open Purchase, before payment settlement: any load-bearing pre-payment dependency failure → FAIL_CLOSED.
- Open Purchase, after payment settlement: evidence persistence/readback/anchoring/indexing failure → EVIDENCE_PIPELINE_INCOMPLETE; reconciliation required; payment MUST NOT be claimed rolled back.
- Protected Job, before verdict settlement: evidence persistence/readback failure → FAIL_CLOSED; no settlement verdict submitted.

## 32.3 Test Assertions

Tests MUST prove:
- successful write and readback;
- deterministic content/hash equality;
- mismatch detection;
- unavailable storage handling;
- no verified-evidence claim when the pipeline is incomplete.
...
| Graph indexing errors at provider selection | FAIL_CLOSED | FAIL_CLOSED |
| Open Purchase post-payment receipt/evidence indexing failure | EVIDENCE_PIPELINE_INCOMPLETE | N/A |
...
## 41.2 Protected Job COMPLETE Proof
...
| v1.2 | 2026-09-12 | Frozen implementation reference. Mode-specific evidence and execution semantics; COMPLETE/REJECT terminology normalized for Protected Job; Open Purchase post-payment evidence failures use EVIDENCE_PIPELINE_INCOMPLETE; Graph/Risk stage boundaries clarified; duplicate IPFS normative text consolidated under Section 24; Registry role clarified (Open Purchase only); Job 186075 classification updated. |
